...
func (mock *S3WriterMock) GetWithPSK(key string, psk []byte) (io.ReadCloser, *int64, error) {
	if mock.GetWithPSKFunc == nil {
		panic("S3WriterMock.GetWithPSKFunc: method is nil but S3Writer.GetWithPSK was just called")
	}
	callInfo := struct {
		Key string
		Psk []byte
...
// Check the length with:
//     len(mockedS3Writer.GetWithPSKCalls())
func (mock *S3WriterMock) GetWithPSKCalls() []struct {
	Key string
	Psk []byte
} {
	var calls []struct {
