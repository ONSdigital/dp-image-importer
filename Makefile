...
VAULT_POLICY:="$(shell vault policy write -address=$(VAULT_ADDR) read-write-psk policy.hcl)"
TOKEN_INFO:="$(shell vault token create -address=$(VAULT_ADDR) -policy=read-write-psk -period=24h -display-name=dp-image-importer)"
